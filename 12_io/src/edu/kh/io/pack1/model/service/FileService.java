...
public class FileService {
	...
	public void method2() {
		...
		if( !file.exists() ) { // 존재하지 않으면
			...
			} catch (IOException e) { //IOException을 잡아서 이런처리를 해주겠다
	...
	public void method3() {
		...
		for( File f : files ) {
			...
			//System.out.println(lastModified); //1727317038658
			
			//java.text.SimpleDateFormat : 간단히 날짜 형식을 지정할 수 있는 객체
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd a h:mn");
														//2024-09-26 오전 11:17
			...
			//->폴더크기를 구하는 메서드는 따로 존재하지 않음
			//->구하려면 폴더안의 모든파일을 순회하여 각 파일 크기를 합산해서 써야함
