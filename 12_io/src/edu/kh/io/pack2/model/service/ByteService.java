...
public class ByteService {
	...
	public void bufferedFileByteOutput() {
		...
		try {
			...
			StringBuilder sb = new StringBuilder();
			sb.append("Hello Hell!!\n");
			sb.append("1234567890\n");
			sb.append("~!@#$%^&*()_+<>?\n");
			sb.append("난 누구 여긴 어디\n");
			sb.append("와 웹개발 너무 어렵고 머리가 어질어질\n");
		...
		public void fileByteInput() {
			...
			try {
				...
				//byte -> 자바에서 정수형
				//다루기 힘들기때문에 정수형 기본형이 int로 변환해서 사용
				...
				while(true) {
					...
					sb.append( (char)value ); //char로 강제형변환 해서 글자형태로 sb 추가
				}
		 ...
		 * 
		 * InputStream의 경우 외부 파일을 읽어올때 사용
		...
		public void bufferedFileByteInput() {
			...
			public void fileCopy() {
				...
				try {
					...
					String target = br.readLine(); //반환하는건 target string이라는 뜻 //입력된 한줄 읽어오기
					...
					// 입력용 스트림 생성
					fis = new FileInputStream(target); //입력용 기반스트림 생성
					bis = new BufferedInputStream(fis); //입력용 보조스트림 생성
