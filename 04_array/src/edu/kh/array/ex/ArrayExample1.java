...
public class ArrayExample1 {//예제 작성용 클래스
	...
	public void ex1() {
		...
		//기본자료형(primitive type):int,double 같은 일반 자료형은 stack 영역에 있음
		//참조자료형(reference type):배열(Array),객체(Object),String는 heap영역에 할당
		//배열요소값 대입
		arr[0]=10;
		arr[1]=50;
		arr[2]=1000;
		...
		//[10,50,1000] 실제로 arr배열이 가진 모든 요소의 값을 알고싶다면..
	...
	public void ex6() {
		...
		String[] arr= {"김밥", "서브웨이", "햄버거", "냉면", "마라탕", "짜장면"};
	...
	public void ex7() {
		...
		//-> ! (Not연산자 : 부정 논리 연산자) 붙여줌으로써 true 로 변경하여
		//{} 안에 있는 코드를 수행하게 함
		if(!flag) { 
	...
	public void ex8() {
		...
		for(int i=0; i <arr.length; i++) {
			if(arr[i].equals(input)) {//String 비교시 ==이 아니라 .equals() 사용
				System.out.println(i+"번째 인덱스에 존재");
				flag= true;	
			}
		}
	...
	// 4 ) count (숫자세기)
		...
		String input = sc.nextLine(); //hello World
		...
		//sc.nextt() ->String 형
		//String.CharAt(0)->String의 0번째 인덱스 문자하나를 추출해서 char형태로 반환
		//"h"->'h'
