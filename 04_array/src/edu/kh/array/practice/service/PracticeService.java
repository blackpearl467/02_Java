...
public class PracticeService {
	...
	int[] arr = new int[9]; //선언+할당
	
	int sum=0; //합계 저장용 변수 for문바깥쪽에써두면됨
	for(int i = 0; i < arr.length; i++) {//배열의길이미만만큼접근
		arr[i]=i+1;//1부터9까지갈수있게
	...
	int [] arr =new int[sc.nextInt()];//사용자가입력한만큼크기가만들어짐
	...
	for(int i =0; i <arr.length; i++) {//for문으로 배열초기화
	 ...
	 * */
	 ...
	 public void practice9() {
		 int[] arr = new int[10];//선언및할당
		 
		 for(int i = 0; i < arr.length; i++) {
			 arr[i] = (int)(Math.random() * 10 + 1);//난수발생
		 }
