...
public class ClientService {
	...
	// 1)서버의 IP주소와 서버가 정한 포트번호를 매개변수로 하여 클라이언트용 소켓 객체 생성
		...
		try {
			...
			// 2)서버와의 입출력 스트림 오픈
			if(clientSocket != null) { //서버와 연결이 성공한경우
				
				is = clientSocket.getInputStream(); //입력용스트림
				os = clientSocket.getOutputStream(); //출력용스트림
				
				// 3)보조 스트림을 통해 성능 개선
				br = new BufferedReader(new InputStreamReader(is));
				//InputStreamReader : 바이트를 문자단위로 입력하는 문자 변환 보조 스트림
